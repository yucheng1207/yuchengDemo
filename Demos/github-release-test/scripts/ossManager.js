...
// @todo - we only upload to oss in production mode right now
const ENDPOINT = "xxx";
const salt = "xxx";
...
class OSSManager {
	 ...
	 */
	async initClient() {
		if (!this.bucket || !this.client || !this.accessKeyId || !this.accessKeySecret || !this.securityToken || !this.expiration || !this.region || this.expiration < Date.now()) {
	 ...
	 */
	getSign(param) {
		const sign = crypto.createHmac('sha1', salt).update(param).digest().toString('base64');
	...
	async getHead(name) {
		...
		const result = await self.client.head(name);
		...
		if (result && result.status === 200) {
			...
			console.log(result);
			throw new Error("Error happening when getting files' head from OSS");
		}
	}
	...
	async listFiles(prefix) {
		...
		do {
			result = await self.client.list(listOptions);
	...
	async deleteFilesOnCloud(key) {
		...
		const result = await self.client.delete(key);
